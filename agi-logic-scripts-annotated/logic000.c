...
if (currentRoom <= 0)
{
  if (!isset(gameRestarted))
  {
    ...
    if (machineType == PC && monitorType != MONO && monitorType != EGA)
    {
...
if (isset(f126) && (isset(f76) || isset(newRoom)))
{
  reset(f76);
...
if (isset(newRoom))
{
  ...
  if (!isset(f48) && !isset(f47) && v44 != 26 && v44 != 25)
  {
...
{
  if (attenuation < 15 && controller(c39))
  {
    ++attenuation;
  }
  if (controller(c38))
...
if (said("restart", "game") || said("restart") || said("begin", "above") || controller(c7))
{
  restart.game();
}
...
if (v44 != 20 && (said("save", "game") || said("save") || controller(c3)))
{
  save.game();
}
...
if (said("examine", "status") || said("status") || said("all") || controller(c10))
{
  status();
  set(f76);
}
...
if (controller(c2) || said("help") || said("help", "alexander"))
{
  if (isset(f43))
  {
...
if (controller(c31) || said("slow"))
{
  set(f125);
  animationInterval = 3;
}

if (controller(c32) || said("normal"))
{
  set(f125);
  animationInterval = 2;
}
if (controller(c33) || said("fast"))
{
  set(f125);
  animationInterval = 1;
}
if (controller(c34) || said("fastest"))
{
  set(f125);
  animationInterval = 0;
}
