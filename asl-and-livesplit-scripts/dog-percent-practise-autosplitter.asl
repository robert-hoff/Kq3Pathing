...
startup
{
  ...
  settings.Add("split-on-map-arrive", true, "Split on arriving at the map pickup point");
}
...
start
{
  if (settings["startonfirstmove"] && current.currentRoom == 7 && current.playerXPos == 110 && current.playerYPos == 123) {
    vars.arrivedAtMap = false;
    return true;
  }
}
...
split
{
  if (settings["split-on-map-arrive"] && !vars.arrivedAtMap && current.currentRoom == 2 && current.playerXPos == 109 && current.playerYPos == 131)
	{
